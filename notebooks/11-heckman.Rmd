...
```{r}
ll_heckman_stata <- function(params, xs, xo, y) {
  ncol_xs <- ncol(xs)
  ncol_xo <- ncol(xo)
  ...
  ll_s <- log(pnorm( -xs[y == 0,] %*% beta_s))
  
  ll_o <- log( pnorm( (xs[y != 0,] %*% beta_s + rho/sigma*(y[y!=0] - xo %*% beta_o)) / (sqrt(1-rho^2)) ) ) - 
    1/2*( (y[y!=0] - xo %*% beta_o)/sigma )^2 - log(sqrt(2*pi)*sigma)
  l <- sum(ll_s) + sum(ll_o)
  l
}
...
ll_heckman_stata(params, xs, xo, y)
ll_heckman(params, xs, xo, y)

```

```{r}
ml <- maxLik(logLik = ll_heckman_stata, start = params, xs = xs, xo = xo,  y= y, method = "BFGS")
summary(ml)
```
